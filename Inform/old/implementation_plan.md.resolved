...
### 3. Schema Simplification (Menus & Sections Merged)
Per the latest feedback, **Menus** and **Sections** overlap significantly in functionality. To streamline the application architecture:
- The `Menu` Prisma model will be entirely removed.
- The `Section` Prisma model will absorb the navigation hierarchy properties (`route`, `roles_allowed`).
- The backend `/menus` endpoints will be deleted and absorbed into the `/sections` endpoints, providing a unified `getTree` method.
- The Admin UI **Menus** tracking page will be deleted. The **Sections** editing page will be enhanced to allow setting routes and role visibility.
- Navigation in the `app/page.tsx` will be wired directly to the newly merged Sections tree instead.
